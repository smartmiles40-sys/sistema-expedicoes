...
function CardInscricao({
  ...
  return (
    <div className={cn("space-y-2 rounded-xl border bg-card p-3", recusada ? "border-border/60 bg-muted/20" : "border-border")}>
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="truncate text-[14px] font-semibold">{it.nome_completo}</div>
          <div className="text-[11px] text-muted-foreground">
            CPF {it.cpf ?? "—"} · {formatDate(it.created_at)}
            {recusada && it.recusada_em ? ` · recusada ${formatDate(it.recusada_em)}` : ` · ${it.status_reserva}`}
          </div>
        </div>
        {recusada ? (
          <Badge variant="critico">Recusada</Badge>
        ) : (
      ...
      <div className="flex items-center gap-2 pt-1">{acoes}</div>
    </div>
  );
}
...
export function InscricoesPendentes({ itens, recusadas }: { itens: InscricaoPendente[]; recusadas: InscricaoPendente[] }) {
  ...
  async function restaurar(it: InscricaoPendente) {
    setBusy(it.id);
    const r = await restaurarInscricao(it.id);
    setBusy(null);
    if (r.ok) { toast.success("Inscrição restaurada para a fila"); router.refresh(); }
    else toast.error("Erro ao restaurar", { description: r.error });
  }

  async function excluir(it: InscricaoPendente) {
    if (!confirm(`Excluir DEFINITIVAMENTE a inscrição de ${it.nome_completo}? Isso apaga os dados e anexos — não dá pra desfazer.`)) return;
    setBusy(it.id);
    const r = await excluirInscricaoDefinitivo(it.id);
    setBusy(null);
    if (r.ok) { toast.success("Inscrição excluída"); router.refresh(); }
    else toast.error("Erro ao excluir", { description: r.error });
  }
    ...
    <div className="space-y-4">
      ...
      {recusadas.length > 0 && (
        <div className="space-y-3">
          <button
            type="button"
            onClick={() => setVerRecusadas((v) => !v)}
            className="flex items-center gap-2 text-[13px] font-semibold"
          >
            <ChevronDown className={cn("h-4 w-4 transition-transform", verRecusadas && "rotate-180")} />
            <Archive className="h-4 w-4 text-muted-foreground" /> Recusadas
            <Badge variant="critico">{recusadas.length}</Badge>
          </button>
          {verRecusadas && (
            <>
              <p className="-mt-1 text-xs text-muted-foreground">
                Ficam guardadas aqui (com os anexos). Dá pra <strong>restaurar</strong> pra fila ou <strong>excluir de vez</strong>.
              </p>
              <div className="grid gap-3 lg:grid-cols-2">
                {recusadas.map((it) => (
                  <CardInscricao
