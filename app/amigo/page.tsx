          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {exp.cartoes_embarque.length > 0 && (
                <div className="mt-2">
                  <div className="mb-1 text-[11px] font-semibold text-[#09282B]/70 dark:text-white/70">Seu cartão de embarque</div>
                  <div className="flex flex-wrap gap-2">
                    {exp.cartoes_embarque.map((c, i) => <IngressoLink key={i} ing={c} onDownload={logVoucher} />)}
                  </div>
                </div>
              )}
...
function DiaRoteiro({ d, destino }: { d: AmigoRoteiroDia; destino: string }) {
  ...
  // Só o passeio do tipo "opcional" SUBSTITUI o dia inteiro ao ser adquirido. O tipo
  // "adicional" nunca assume o dia — mas continua sendo OFERECIDO (com WhatsApp) e, se
  // adquirido, aparece como "adquirido" sem tomar o dia.
