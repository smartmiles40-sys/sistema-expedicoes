...
/**
 * Cria/atualiza um passageiro a partir do payload do Bitrix (upsert por
 * `bitrix_deal_id`). Compartilhado entre `/api/bitrix/passageiro-sync` (payload já
 * pronto) e `/api/bitrix/sync-contato` (que traduz o contato cru antes de chamar).
 */
export async function upsertPassageiroBitrix(data: SyncData): Promise<UpsertPassageiroResult> {
  ...
  const { data: existing } = await supabase
    .from("passageiros").select("id").eq("bitrix_deal_id", data.bitrix_deal_id).maybeSingle();
  const action: "created" | "updated" = existing ? "updated" : "created";

  const { data: result, error } = await supabase
    .from("passageiros").upsert(upsertPayload, { onConflict: "bitrix_deal_id" }).select("id").single();
  ...
  return { ok: true, passageiro_id: r.id, action };
}
