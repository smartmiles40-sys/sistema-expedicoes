...
// Campos do Perfil que "seguem a pessoa" (pré-preenchem). "o que te anima",
// "significado especial" e a Conexão (acompanhante) ficam de fora — são
// respondidos a CADA expedição.
...
export function agregarPerfil(linhas: Pax[]): Partial<Pax> | null {
  ...
  // Perfil do viajante: campos de CAMPOS_PERFIL_CARRY seguem a pessoa.
...
export function montarValores(base: Partial<Pax>, existente: Pax | null): ValoresInscricao {
  ...
  return {
    ...
    // Conexão (acompanhante): sempre em branco — respondida a cada expedição.
    acompanhante_nome: "",
